...
<html>
<head>
    <title>Product_search_list</title>
        ...
        <div class="pagination"><!--有上一页， 无上一页， 有下一页， 无下一页 -->
            ...
            <c:if test="${bean.currentPage != 1}">
                <li class="disablepage_p">
                    <a class="disablepage_a" href="${pageContext.request.contextPath}/menuSearchServlet?currentPage=${bean.currentPage - 1}&category=${bean.category}"></a>
                </li>
            </c:if>
            <!--无上一页，-->
            <c:if test="${bean.currentPage == 1}">
            ...
            <c:forEach begin = "1" end="${bean.totalPage}" var="pageNum" >
                ...
                <c:if test="${pageNum != bean.currentPage}"> <!-- 不是当前页 -->
                    <li><a href="${pageContext.request.contextPath}/menuSearchServlet?currentPage=${pageNum}&category=${bean.category}">${pageNum}</a></li>
                </c:if>
            </c:forEach>
            <!--有下一页-->
